...
RUN yum clean all && yum install -y epel-release && yum install -y python-pip supervisor nginx uwsgi uwsgi-plugin-python && rm -rf /var/cache/yum/* && pip install puppetboard==$PUPPETBOARD_VERSION && rm -rf /root/.cache
